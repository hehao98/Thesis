...
The excluded projects are precisely those for which pinning would force the most duplicates, the strongest crossover in floating versions, the most update lock-in, and the most unpatched vulnerabilities, so the reported effects on \Code{n\_bloated}, \Code{n\_floating}, \Code{n\_outdated\_deps}, and \Code{n\_vuln} are lower bounds on the true costs of pinning, with the magnitude of the underestimation increasing in dependency graph size $G$.
The effect on \Code{n\_auto\_updates} is structural---pinning sets direct-dependency auto-updates to zero by construction---so the bias on this outcome is plausibly small and of indeterminate sign.
