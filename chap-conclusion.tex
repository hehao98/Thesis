...
In each case, an assumption that an earlier study could only state became one that a later design could enforce or test, and a later design can recognize which newly available data lifts which assumption only if the assumption was articulated in the first place.
The residual threats named in the three credibility assessments---simulation fidelity in Chapter~\ref{chp:pinning}, reverse causality between organic momentum and purchase decisions in Chapter~\ref{chp:fake-stars}, and time-varying shocks that coincide with tool adoption in Chapter~\ref{chp:cursor}---occupy the same position today: They are the disclosed targets that a future improved design can address.
