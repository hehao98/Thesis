...
None of these steps is available when the assumptions remain implicit in a model specification, because an undisclosed assumption cannot be deliberately relaxed.
The three empirical chapters show how quickly disclosed targets become reachable, since each benefits from infrastructure that did not exist a decade ago.
...
In each case, an assumption that an earlier study could only state became one that a later design could enforce or test, and a later design can recognise which newly available data lifts which assumption only if the assumption was articulated in the first place.
The residual threats named in the three credibility assessments---simulation fidelity in Chapter~\ref{chp:pinning}, reverse causality between organic momentum and purchase decisions in Chapter~\ref{chp:fake-stars}, and time-varying shocks that coincide with tool adoption in Chapter~\ref{chp:cursor}---occupy the same position today: They are the disclosed targets that a future design with richer panels or better instruments can address.
...
\subsection{On Practitioners' Belief and Impact of Empirical SE Research}
