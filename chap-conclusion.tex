...
Practitioners form their beliefs primarily from personal experience rather than from published evidence~\cite{DBLP:conf/icse/Devanbu0B16, DBLP:conf/esem/PassosBCM11}, they are persuaded more readily by local opinion than by empirical findings~\cite{DBLP:conf/isese/RainerHB03}, and their beliefs are not consistently supported by data from the projects that formed them~\cite{DBLP:journals/ese/ShrikanthNFM21}.
The pinning chapter's estimate---a security benefit that reverses in dependency graphs of 498 nodes or more---is a population-level, conditional quantity that neither the security cohort nor the maintenance cohort in the prior debate held a belief about, and the same is true of the two-month promotional horizon in the fake-stars chapter and the transient velocity gain in the Cursor chapter.
...
Research supplements experience because the two sample the data-generating process differently.
...
The pinning debate illustrates the complementarity: The security cohort and the maintenance cohort each generalized from the projects they had seen, both were right within their slice, and the dependency-graph-size condition that reconciles them is visible only from an ecosystem-scale sample.
An estimate that matches no prior is therefore the information the study adds.
A practitioner who combines the two sources arrives at a belief that neither source could have produced alone, and the impact of a study is the size of that update.
...
This is the discounting Chapter~\ref{chp:intro} describes: When a controversial finding fails to rule out alternative explanations, practitioners become skeptical of it altogether, and that skepticism is warranted.
Steps 3 and 4 of the framework---bounding residual biases and navigating alternative causal structures---are where alternatives are made improbable, so a causal credibility assessment is, in Bayesian terms, a report of the likelihood ratio the study is entitled to claim.
Iterative refinement compounds that evidence: Each subsequent study that targets an assumption the previous one left open removes an alternative under which the earlier evidence would have been equally likely, so the accumulated likelihood ratio across a research program grows in a way that a sequence of correlational studies restating the same association under the same unaddressed alternatives cannot.
...
Three qualifications keep this position from overreaching.
First, the goal is calibration, not persuasion in a predetermined direction.
...
All three chapters end with a conditional finding that partially vindicates the belief they set out to challenge, and the framework must remain equally willing to report that a myth survives.
Second, practitioners hold heterogeneous priors, so there is no single threshold of extraordinary evidence; the threshold is set by the most skeptical reasonable prior, and the conditional claims discussed above are what let a study address each prior at its own resolution.
For a practitioner whose belief concerns a condition the study does not analyze, the honest answer is that the study does not speak to them, and the estimand definition in Step 2 makes this boundary explicit rather than leaving it to be inferred.
Third, the belief update rarely happens through the paper itself.
...
Beliefs travel through intermediaries---security scorecards, tool defaults, platform policies, educators, and colleagues---and the audience whose beliefs a study most needs to move is often the maintainer of an OpenSSF Scorecard check or the designer of a package manager's default resolution rule rather than the individual developer.
The communicative role of the framework discussed above matters most for these intermediaries: An intermediary that encodes a claim with more confidence than the evidence supports reproduces the myth in a new form, and an explicit credibility assessment is what gives the intermediary a basis for calibrating the confidence it encodes.

\subsection{Closing Posture: How the Field Should Move Forward}
\label{sec:conclusion-closing}

The thesis has argued that causal credibility assessment is feasible; whether the field moves as a result is a separate question, and there are two symmetric ways it could go wrong.
...
Both failure modes have precedents in fields that adopted causal inference earlier than software engineering, and their experience shapes the position taken here.

The identification-police concern is the one most often raised against this thesis, and the economics profession's experience supplies the answer.
...
The framework does not disarm criticism; it directs criticism to a target and gives it a burden.
This is also why an empirical standard is the right institutional response.
...
The field should also recognize that it already has a police of the opposite kind.
...
Replacing the association police with an identification police would exchange one reflex for another; the standard replaces both with an argument about assumptions.

The inertia concern is the more serious of the two, and the thesis's own evidence supports taking it seriously.
Use of causal methods among colleagues in my immediate research community has visibly increased, but a thesis on causal credibility cannot rest a prediction on that observation, and the flat adoption line in Chapter~\ref{chp:background} is the more credible datum.
Medicine offers the sharper warning: Evidence-based medicine has had a mature evidence base and formal guidelines for three decades~\cite{sackett1996evidence}, and a substantial literature documents that well-evidenced guidelines do not change practice wholesale.
...
Inertia is the barrier that none of these moves individually, and it yields only when outcome expectancy and venue norms move together.

Beyond targeting barriers, three commitments distinguish the position this thesis takes from the adoption of a method.
First, the standard the field should set is one of \emph{disclosure}, not of design: A study that asks a causal question states what its evidence supports, under which assumptions, and with what residual threats, and a study that does not ask a causal question says so and writes its implications accordingly.
The standard applies to the 29\% of empirical SE papers with a causal motivation and leaves the rest untouched, and it guards against the ritual adoption that the field has already experienced with qualitative methods~\cite{DBLP:journals/tosem/RobillardAEGLNNSSS24} by asking whether an assumption is plausible in context rather than whether its name appears.
Second, the unit of progress is a research program rather than a paper.
Section~\ref{sec:conclusion-practitioner-beliefs} argued that credibility compounds across studies only when each study targets an assumption the previous one left open; replication that repeats the same design under the same unaddressed alternatives adds precision but not credibility, and the field's replication efforts should be organized accordingly.
Third, software engineering holds a structural advantage over medicine in the last mile from evidence to practice, and the field should use it.
A clinical guideline must persuade each physician; a software engineering finding can be encoded in a tool default, a scorecard check, a package manager's resolution rule, or a platform policy, and the intermediaries identified in Section~\ref{sec:conclusion-practitioner-beliefs} change practice at ecosystem scale without persuading each practitioner individually.
The pinning chapter's finding is actionable through the OpenSSF Scorecard's pinned-dependencies check and npm's default version range far more directly than through any number of developers reading the paper, and the same holds for platform moderation of fake stars and for the defaults that AI coding tools ship with.
The corresponding obligation is that the confidence an intermediary encodes must match the credibility the evidence supports, which is the calibration the framework exists to make inspectable.
