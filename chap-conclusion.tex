...
The contributions of each chapter are summarized below; the cross-chapter comparison in Section~\ref{sec:conclusion-comparison} makes their relative causal credibility explicit.
...
A literature review of 5{,}341 main-track papers at ICSE, FSE, and ASE between 2015 and 2025 documents a structural gap in the field: 29\% of empirical studies are motivated by a causal research question, but only 1.9\% employ a recognized causal inference method, and the ratio has remained flat for a decade.
...
A complementary panel generalized method-of-moments analysis decomposes the velocity-quality feedback and shows that the accumulated technical debt subsequently dampens future development velocity.
...
The three empirical chapters apply the same four-step framework but achieve distinctly different levels of causal credibility, governed by what the research settings and treatment assignment mechanisms permit and extended to the author's best rigor in their analyses.
...
Table~\ref{tab:conclusion-comparison} summarizes the comparison along the four dimensions of the framework.
...
The thesis statement claims \emph{feasibility}.
...
For misciting downstream readers, the framework cannot police behavior, but it makes the gap between what a study claims and what a downstream reader infers visible and challengeable.
...
The shift the thesis advocates is therefore one of expectation rather than of method: A study that claims a causal effect should be expected to name the assumptions the claim rests on, just as a randomized trial is expected to report its compliance rate.
...
For a practitioner whose belief concerns a condition the study does not analyze, the researcher must provide the honest answer that the study does not speak to them and disclose the boundaries transparently.
...
The identification-police concern is the one often raised against this thesis and the economics profession's experience may supply the answer.
...
The inertia concern is the other flip of the coin which I believe the experience from Medicine can help shape an answer.
Evidence-based medicine has had a mature evidence base and formal guidelines for three decades~\cite{sackett1996evidence}, and a substantial literature documents that well-evidenced guidelines do not change practice wholesale.
...
Overall, inertia is the barrier that none of these moves individually, and it yields only when awareness, self-efficacy, outcome expectancy, and venue norms move together.\footnote{
...
It claims that the disconnect is bridgeable through an explicit articulation of causal theory and assumptions, and that the four-step framework is one usable operationalization of that bridge.
