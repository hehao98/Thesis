...
\renewcommand{\arraystretch}{1.1}
    \footnotesize
    \centering
    \caption{The argued trade-offs ({\color{darkgreen}\faToggleDown} benefits and {\color{darkred}\faToggleUp} drawbacks) between pinning and floating}
    \begin{tabular}{l@{~}p{11cm}ll}
    \toprule
        \multicolumn{2}{l}{The Trade-Offs (\faLock~~Security and {\faCogs}~~Maintenance)} & Floating & Pinning  \\
    \midrule
        \faLock & \textbf{Attack Surface for Malicious Package Updates}~\cite[e.g.,][]{openssf-pinning, facts-vs-feelings, DBLP:journals/ieeesp/ZahanKHSW23, DBLP:conf/sp/LadisaPMB23} & {\color{darkred}\faToggleUp}~Larger & {\color{darkgreen}\faToggleDown}~Smaller \\
        ...
        DBLP:journals/tse/JafariCAST22, DBLP:journals/jss/RaemaekersDV17, DBLP:journals/tosem/BogartKHT21} & {\color{darkred}\faToggleUp}~More & {\color{darkgreen}\faToggleDown}~Less  \\[.5em]
        \faLock & \textbf{Attack Surface from Security Vulnerabilities}~\cite[e.g.,][]{floating-discussion, should-i-pin-python-deps, DBLP:journals/tse/JafariCAST22, DBLP:journals/ese/ChinthanetKMIIM21, DBLP:journals/tse/0038SP00C0023} & {\color{darkgreen}\faToggleDown}~Smaller & {\color{darkred}\faToggleUp}~Larger \\
        ...
        DBLP:journals/tse/JafariCAST22} &  {\color{darkgreen}\faToggleDown}~Less & {\color{darkred}\faToggleUp}~More \\
        \faCogs & Effort to Resolve Dependency Conflicts~\cite[e.g.,][]{floating-discussion} %should-i-pin-python-deps
         & {\color{darkgreen}\faToggleDown}~Less & {\color{darkred}\faToggleUp}~More \\
    \bottomrule
    \end{tabular}
...
Given this argument, together with prior arguments that pinning increases application stability and reproducibility~\cite{pin-your-npm-yarn-dependencies, stop-version-range, pin-exact-dep-versions}, many practitioners now argue in favor of pinning for security-sensitive applications---for example, a survey of 17 domain experts and 134 software developers by \citet{DBLP:conf/sp/LadisaPMB23} ranks pinning as a top-3 safeguard against supply chain attacks in terms of utility/cost ratio; the OpenSSF Scorecard project~\cite{openssf-pinning, DBLP:journals/ieeesp/ZahanKHSW23} included ``pinned dependencies'' as one of its 18 security best practices.
...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{0.46\linewidth}
        ...
        \caption{The distribution of versioning strategies.}
...
Overall, controversies remain on whether the benefits of pinning outweigh its drawbacks, as manifested by online discussions~\cite{facts-vs-feelings, renovate-pinning, pinning-only-work} and evidenced by the contradicting results from the collected practitioner opinions in prior studies \cite{DBLP:journals/tse/JafariCAST22, DBLP:conf/sp/LadisaPMB23}.
...
\paragraph{Chapter Overview.} The remainder of this chapter presents quantitative evidence on the security and maintenance impact of version constraints in a packaging ecosystem, especially in light of recent concerns about malicious package updates.
The findings align with the opinions of npm developers~\cite{DBLP:journals/tse/JafariCAST22} but contradict the views of security practitioners~\cite{DBLP:conf/sp/LadisaPMB23, DBLP:journals/ieeesp/ZahanKHSW23}, suggesting that pinning direct dependencies is futile (and can be even counter-productive) as protection against malicious package updates.
The chapter also presents evidence that a new ecosystem-wide intervention, along with tooling changes and ecosystem-wide allocation of collectively pooled resources, can be an effective intervention against malicious package updates.
These findings lead to concrete advice for developers and ecosystem policy makers to improve the state of software supply chain security (Section~\ref{sec:pinning-discussion}).
Within this thesis, the chapter challenges the software engineering myth that pinning dependency versions improves project security against malicious package updates, and Section~\ref{sec:pinning-credibility} applies the four-step causal credibility assessment introduced in Section~\ref{sec:thesis-statement} to make explicit the causal theory and assumptions on which this evidence rests.

\section{Background and Related Work}
\label{sec:pinning-background}
...
We modified version constraints in the original file to illustrate different types of version constraints defined in Section~\ref{sec:method-rq1}. Other details irrelevant to this chapter are omitted.}
...
It is worth noting that the above discussion applies not only to floating direct dependencies but also to floating transitive dependencies, even if the developer pins all their direct dependencies.
...
However, these (supposed) security best practices may interact with existing \emph{software engineering} trade-offs, among which the pinning versus floating problem is particularly complicated (Table~\ref{tab:trade-offs});  the strongly held convictions from both sides lead to a considerable amount of controversy among practitioners~\cite{facts-vs-feelings, renovate-pinning, pinning-only-work}.

\paragraph{Novelty of the Investigation.}
...
The changed threat landscape and the complexity of this pinning versus floating problem eventually lead to controversies and divergence in practitioner opinions~\cite{facts-vs-feelings, renovate-pinning, pinning-only-work, DBLP:conf/sp/LadisaPMB23, DBLP:journals/tse/JafariCAST22}.
Prior studies are either collecting these opinions~\cite{DBLP:journals/tosem/BogartKHT21, DBLP:journals/tse/JafariCAST22, DBLP:conf/sp/LadisaPMB23} or aggregating descriptive statistics from software repositories~\cite{DBLP:conf/msr/0001PSTB19, DBLP:conf/msr/PinckneyCGB23, DBLP:journals/tse/DecanM21, DBLP:journals/jss/RaemaekersDV17}; to the best of our knowledge, no prior study has attempted to quantify the impact of version constraints using statistical modeling methods.
...
This forms the main motivation of the chapter.
...
As we discussed above, dependency pinning is often acknowledged as a security best practice to reduce the attack surface for malicious package updates~\cite{DBLP:conf/sp/LadisaPMB23, DBLP:journals/ieeesp/ZahanKHSW23}, but controversies remain on whether the security benefits of pinning outweigh its maintenance drawbacks~\cite{facts-vs-feelings, renovate-pinning, pinning-only-work, DBLP:conf/sp/LadisaPMB23, DBLP:journals/tse/JafariCAST22}.
...
Hence, the goal of RQ1 is to empirically measure the actual security and maintenance impact of this intervention.
...
However, the resurgence of pinning since 2018 indicates that an increasing number of developers have encountered issues with floating and started to use pinning instead, as also indicated in the gray literature~\cite{floating-discussion,
...
\Code{n\_vuln}, \Code{n\_outdated\_deps} and \Code{n\_bloated} should decrease with floating and increase with pinning. We designed all metrics such that lower values are generally better (e.g., corresponding to lower costs or risk).
...
In this chapter, all outcome metrics are estimated from the conservative case of no manual developer actions; the exploration of other cases is left to future work.
...
In terms of external validity, it is important to note that our study is specific to the npm ecosystem and the results may not generalize to other ecosystems that adopt different baseline practices.
...
Note that since our goal is explanatory (not predictive), we do not evaluate models based on R$^{2}$ values, but conduct a series of robustness checks to see whether the findings are robust over different settings (Section~\ref{sec:results-rq2}).
...
Given the set $P$ of all packages in the network, we quantify the impact of compromising a package $a\in P$ as the download-weighted coverage of its reachable dependents within the entire npm network, where every package that depends directly or indirectly on package $a$ is considered reachable if the edge to $a$ in its dependency graph is floating:
\begin{equation}
\label{eq:impact}
...
\begin{figure}[t]
    ...
    \end{subfigure}
    \caption{Examples to illustrate attack impact computation and local/transitive pinning.}
    \label{fig:example-rq3}
\end{figure}


\paragraph{Pinning Mechanism.}
With npm's existing mechanisms, the maintainers of the $n$  defending packages can only pin their direct dependencies (\textit{local pinning}), but not their transitive ones. 
While maintainers can freeze their dependency graph with a \Code{package-lock.json} file, it only affects their own package, not anyone importing their package, with npm's current design.
...
\begin{figure}[t]
    ...
    \caption{The reduction of malicious package update risk ($risk(A)$) by securing top $n$ packages with five defense selection strategies using local pinning and transitive pinning, respectively.}
...
\input{analysis-pinning}

\section{Discussion}
\label{sec:pinning-discussion}

\subsection{Why can pinning direct dependencies lead to a larger total number of floating dependencies (i.e., a larger attack surface for malicious updates)?}
...
\label{dedup}
This surprising effect is explained by a detail of how the npm dependency resolver handles dependency conflicts~\cite{how-npm3-works} (cf. Section~\ref{sec:pinning-background}):
...
In the npm ecosystem, it means to not only use but also \emph{commit} the \Code{package-lock.json} file (Section~\ref{sec:pinning-background}) to the version control repository (44.21\% of GitHub repositories in our dataset did this).
...
Even if only a small number of maintainers opt to pursue this (i.e., transitive pinning and extensively review their updates), it can have a large impact on the ecosystem.
...
This chapter has presented a counterfactual analysis and simulation study on the impact of version constraints in the npm ecosystem.
The chapter explored the security and maintenance impact of locally pinning direct dependencies and the effectiveness of pinning on ``important'' packages against possible malicious package updates.
The findings provide counter-evidence for the use of local pinning as a security best practice, but support the effectiveness of coordinated pinning in critical open-source packages, leading to a series of implications for the design of dependency management tools and ecosystem-level coordination to enhance software supply chain security.
With respect to the thesis statement in Section~\ref{sec:thesis-statement}, the chapter does not dispel the pinning myth outright: The causal credibility assessment in Section~\ref{sec:pinning-credibility} specifies the conditions---dependency graph size and the pinning decisions made upstream---under which pinning protects against malicious package updates and the conditions under which it does not, and identifies simulation fidelity as the one assumption on which this reading rests.
A replication package is available at \url{https://doi.org/10.5281/zenodo.14693233}.
