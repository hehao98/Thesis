...
\begin{figure}[t]
    ...
    \node[ynodeF, right=0.35cm of Q2] (Vnext) {$Y_2$: lines added$_{i,t+1}$};
    ...
    \draw[gmmedge] (Q2.east) to[bend right=15] node[below, font=\tiny\itshape] {GMM: $C_t{\to}L_{t+1}$} (Vnext.west);
    \end{tikzpicture}
    ...
    Cumulative codebase size (orange) is promoted to an explicit moderator: the chapter's GMM analysis (Section~\ref{sec:finding-interaction}, Table~\ref{tab:causal-paths}) shows that the volume-mediated path $V_t \to \mathrm{LoC}_t \to Q_t$ absorbs most of the warnings effect and a large share of the complexity effect, while a direct $D \to$ per-line-complexity $\to C_t$ path remains after this control.
...
Cumulative lines of code is promoted to an explicit moderator on the DAG because the chapter's GMM decomposition (Table~\ref{tab:causal-paths}) shows that the volume path absorbs most of the warnings effect and a large share of the complexity effect, while a residual direct effect on complexity survives the lines-of-code control and motivates the chapter's headline interpretation.
...
The GMM analysis targets the contemporaneous causal effect of one outcome on another conditional on Cursor adoption, repository identity, and calendar time, plus the lagged effect of one outcome on next-period values of another.
The four estimands of interest are listed in Equation~\ref{eq:temporal} and the corresponding regression in Equation~\ref{eq:dynamic-gmm}.
