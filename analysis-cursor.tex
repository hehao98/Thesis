...
\begin{figure}
    ...
    \draw[cedge] (Resid.east) -| ([xshift=-0.6cm]V1.west) -- (V1.west);
    \draw[cedge] (Resid.east) -| ([xshift=-0.6cm]V2.west) -- (V2.west);
    \draw[cedge] (Resid.east) -| ([xshift=-0.34cm]Q1.west) -- (Q1.west);
    \draw[cedge] (Resid.east) -| ([xshift=-0.34cm]Q2.west) -- (Q2.west);
    ...
    The principal assumptions are A1 (parallel trends) for the DiD estimand and B1 (sequential exogeneity) for the GMM estimand.}
...
Team composition and contributor expertise predict both adoption and outcomes, with documented heterogeneity in how developers of different skill levels benefit from AI assistance~\cite{DBLP:journals/jss/DakhelMNKDJ23, DBLP:journals/corr/abs-2302-06590, cui2025effects}.
...
The assumption nevertheless remains untestable in the post-period and is the principal condition for the DiD estimate.
...
The borderline cases---repositories with shared maintainers, organization-level rollouts where a developer adopts Cursor across several projects simultaneously---do not admit a clean construction-based argument and are discussed under Step~3.

\paragraph{A4. Stable treatment definition.}
The treatment ``Cursor adoption'' has a single, well-defined operationalization across the panel.
...
The estimate is best read as the ATT of \emph{deliberate, visible adoption of Cursor as a development practice}, averaged over the realized heterogeneity of how repositories use the tool.
Generalization to specific adoption intensities is an external-validity concern and is handled in the existing Limitations subsection (Section~\ref{sec:cursor-limitations}).
...
Direction of bias depends on the confounder; the most plausible scenario is a ``modernization push'' that bundles tool adoption with pace acceleration, which would reinforce the velocity-up and quality-down pattern, so the central findings are robust in sign but possibly inflated in magnitude.
...
They are addressed in the existing ``Limitations and Threats to Validity'' subsection (Section~\ref{sec:cursor-limitations}) and the ``Robustness Checks'' subsection (Section~\ref{sec:cursor-robustness}); we do not re-litigate them here.
...
Five of them are already addressed by the existing chapter through dedicated robustness checks (Section~\ref{sec:cursor-robustness}, Figure~\ref{fig:robustness-checks-all}) and the cross-estimator comparison (Section~\ref{sec:appendix-alternative-estimators}); we summarize their resolution and pointers in the table below.
...
\midrule
The transient velocity finding is an artifact of OSS repository mortality rather than Cursor. &
\emph{Active Months} ($>0$ commits) and \emph{Very Active} ($\geq 10$ commits) subset robustness checks (Section~\ref{sec:cursor-robustness}, Figure~\ref{fig:robustness-checks-all} Row~2) show the velocity-then-decay pattern persists in the active sample. \\
\addlinespace
The pattern reflects an excitement--frustration--abandonment cycle rather than a sustained Cursor effect. &
\emph{High Contributor Adoption} and \emph{Cursor Configuration Changes} subsets (Section~\ref{sec:cursor-robustness}, Figure~\ref{fig:robustness-checks-all} Row~1) show velocity gains \emph{persist} under sustained usage; explicit treatment in Section~\ref{sec:cursor-discussion}. \\
\addlinespace
...
The \emph{Cursor and Other} versus \emph{Only Cursor} subset robustness checks (Section~\ref{sec:cursor-robustness}, Figure~\ref{fig:robustness-checks-all} Row~3) show main findings amplify in the cleaner subset. The detection-signature limitation is acknowledged in Step~3 above. \\
\addlinespace
The findings are driven by particular programming languages or by particular adoption cohorts. &
Per-language robustness panels (Section~\ref{sec:cursor-robustness}, Figure~\ref{fig:robustness-checks-all} Row~4) show qualitative consistency across JavaScript/TypeScript, Python, and Go; the cross-estimator appendix (Section~\ref{sec:appendix-alternative-estimators}) reports the cohort-disaggregated \citet{callaway2021difference} estimator and discusses divergence on quality outcomes honestly in Section~\ref{sec:estimator-comp}. \\
\bottomrule
\end{tabular}
\end{table}
...
This shock cannot be separated from the treatment effect by any observational design, and is the principal assumption the chapter's findings ultimately rest on.
The most plausible candidate scenarios---a ``modernization push'' that bundles tool adoption with pace acceleration, or a project entering a feature-development phase that coincides with Cursor adoption---would reinforce the velocity-up and quality-down pattern, so the central findings are robust in sign but possibly inflated in magnitude.
...
\label{sec:cursor-credibility-summary}

The credibility of the chapter's causal interpretation rests primarily on the residual portion of A1 (no time-varying coincident unobservables) for the DiD estimand and on B1 (sequential exogeneity) for the GMM estimand.
...
The chapter's discussion and practical recommendations (Section~\ref{sec:cursor-discussion}, Section~\ref{sec:practical-implications}) survive this scrutiny.
The most plausible residual confounder---a modernization push that bundles Cursor adoption with pace acceleration or with the arrival of an experienced contributor---reinforces both the velocity-up and the quality-down pattern, so the call for quality assurance to scale with AI-era velocity is robust in sign even if individual coefficient magnitudes are inflated.
Mid-window adoption of competing AI tools rebases the estimate from a no-AI counterfactual to a partly-AI baseline, which sharpens rather than weakens the qualitative recommendation that any AI-assisted development setting needs proportionally stronger quality processes.
Pre-commit experimentation attenuates the immediate post-adoption velocity gain, so the reported transient velocity effect is best read as a conservative lower bound, and the velocity-then-decay narrative survives at full strength.
The GMM-based finding that code complexity exhibits a per-line effect surviving the volume control---which underpins the call for readability-aware tooling and complexity-triggered refactoring---rests on B1 and on the cross-direction agreement of the four GMM specifications, both of which the chapter defends through the Arellano-Bond AR(2) and Sargan diagnostics and the cross-coefficient sign pattern (Table~\ref{tab:causal-paths}).

\begin{summary-rq}
\textbf{Causal credibility, in plain language:} The Cursor study's design is relatively strong but not the strongest in this thesis.
While parallel trends and sequential exogeneity remains untestable and not enforced by construction, the design is enhanced by matching, the Borusyak imputation, and cross-direction GMM agreement that substitutes for the missing experimental variation.
The chapter's recommendation that quality assurance must scale with AI-era velocity survives this scrutiny because the most plausible residual confounders either reinforce the velocity-up and quality-down pattern or attenuate the reported velocity gains.
\end{summary-rq}
