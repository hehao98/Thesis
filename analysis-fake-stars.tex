...
The DAG in Figure~\ref{fig:dag-fake-stars} makes that assumption visible alongside the residual confounding paths that threaten it; the subsequent subsections name the assumption, identify the residual threats to causal credibility, and report the executed robustness checks that probe the plausibility of these residual threats.
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        ...
        confbox/.style={draw, rounded corners, fill=gray!12, align=left, text width=4.5cm, inner sep=4pt},
    ...
    \node[rbox, below=0.7cm of D] (Z) {%
        ...
        (e.g., LLM surge in 2024, crypto surge in 2021--2022)
        not absorbed by $\mu_i$ or $\lambda_t$ alone
    };
    ...
    %\draw[cedge] (Conf.north east) to[bend left=30] (Y.north);
    \draw[->, red!70!black, semithick] (Z.north) to[bend right=0] (D.south);
    \draw[->, red!70!black, semithick] (Z.east) to[bend right=30] (Y.south);
    \draw[revedge] (Y.north) to[bend right=35] node[above, font=\tiny\itshape] {reverse $(+)$: $\textit{real}_{i, t-k} \to D_{i,t}$} (Dlat.east);
    \end{tikzpicture}%
    }
    ...
    The principal assumptions are A1 (strict exogeneity), A2 (no unobserved time-varying confounders), and A5 (panel completeness).}
...
Finally, the DAG draws an explicit reverse-causality edge from  real stars growth $\textit{real}_{i, t-k}$ to fake star purchase decision $D_{i,t}$ later.
...
Let $Y_{i, t+k}(d)$ denote real stars gained by repository $i$ in month $t+k$ if the maintainer had purchased $d$ fake stars in month $t$, holding latent fake star purchase decision $D_{i, s}$ fixed at observed values for all $s \neq t$.
...
``Buying fake stars'' has a single, well-defined operationalization across the panel.
...
The estimate is best read as the average elasticity over the realized heterogeneity of detected campaigns; generalization to specific strategies is an external-validity concern handled in the existing Limitations subsection (Section~\ref{sec:limitations}).
...
The discussion's counterfactual claim (``buying fake stars is ineffective'') generalizes only to the extent that this average partial effect is informative about the population of practitioners who would consider buying.
...
Upon revisiting the study with the proposed framework, four threats to the internal causal validity of the RQ4 estimate remain.
...
Repository fixed effects absorb only time-invariant traits, and a single calendar-time fixed effect $\lambda_t$ cannot absorb interactions between topic and time.
A hype wave in a particular topic raises both the demand for fake stars and the rate at which real users discover and star repositories within that topic, opening a back-door path from the latent purchase decision to the outcome that runs through unobserved attention.
Direction of bias on the short-run $\hat\beta_1$ is upward whenever hype acts as such a common cause; the magnitude depends on how concentrated the panel is in hyped topics during hyped time windows, which the chapter's RQ3 analysis documents to be substantial.
...
A maintainer who sees real-star growth stalling may buy stars to reignite visibility (``panic-buy''); a maintainer who sees real-star growth accelerating may buy stars to amplify the momentum (``hype amplification'').
Either dynamic opens a back-door path from the past outcome through the latent decision to the observed proxy, and the lagged-dependent-variable specification only partially blocks it because of small-$T$ Nickell bias~\cite{nickell1981biases}.
The two stories carry opposite signs of bias on the headline $\hat\beta_1$---panic-buying biases it downward, hype amplification biases it upward---so the panel-FE estimator alone cannot bound even the direction of the resulting distortion without external evidence on which dynamic dominates.
...
The surviving subset is not a random slice of campaigns---it is precisely the repositories whose maintainers (and GitHub) kept them alive long enough to be observed at horizons of three months or more, while the deleted majority drops out of the regression entirely.
This makes the long-run negative coefficient on $\log \textit{all\_fake}_{i, t-3}$ ambiguous between two readings.
A real user-disengagement effect is consistent with the sign: over time, real users notice the inflated history and stop starring.
A mechanical selection artifact is equally consistent: the repositories with the largest fake-star histories tended to be deleted earliest, so the cases pulling the long-run coefficient negative are precisely those whose campaigns failed to convert into sustained attention before exit.
Direction of bias on the short-run $\hat\beta_1$ is similarly unclear a priori---upward if repositories with successful short-run lift survive longer, downward if survival is dominated by failure---and the panel-FE estimator alone cannot separate the two channels.

\paragraph{Cumulative-real series is monotone non-decreasing by construction.}
Another separate sub-mechanism of survivorship concerns whether GitHub retroactively removes stars from heavily faked repositories, generating part of the long-run negative coefficient through platform action rather than user behavior.
...
We pair the first three with a concrete robustness check that operationalizes the response, and explain why the fourth is not amenable to a within-sample test.
We summarize the headline coefficients in Table~\ref{tab:fake-stars-robustness}. %; full scripts and results are available in the Thesis repository \Code{https://github.com/hehao98/Thesis}.
The three executed checks all re-fit the AR(2) fixed-effects specification of Table~\ref{tab:regression}, varying either the sample or the regression equation:
\textbf{R1 (survivor-length stratification)} addresses the survivorship concern from Alternative~1 by computing each repository's panel length as a proxy for eventual deletion (the dataset does not flag deletion directly) and refitting AR(2) FE separately on long-survivors (panel length $\geq 12$ months) and short-survivors (panel length $< 12$ months);
\textbf{R2 (topic stratification)} addresses the topic-momentum concern from Alternative~2 in two complementary slices, refitting AR(2) FE first on three calendar-period subsets (pre-2024, the 2024 LLM surge from January to August, and post-surge) and then within each manually labeled \Code{domain} of the 580-repository labeling exercise of Section~\ref{sec:rq3} (\Code{repo\_labels.csv}; 375 of the 580 also appear in the AR(2) panel, with strata smaller than 30 repositories or 100 panel observations omitted as too small for stable within-FE estimation);
\textbf{R3 (lead-lag swap regression)} addresses the reverse-causality concern from Alternative~3 by swapping the dependent variable to $\log \textit{fake}_{i,t}$ and refitting AR(2) FE with lagged real-star intensity as the regressor, so that the sign of the lagged-real coefficients distinguishes panic-buying (negative) from hype amplification (positive).

\paragraph{Alternative 1: The long-run negative coefficient is a survivorship artifact.}
...
Robustness check R1 reports the negative long-run coefficient persisting in long-survivors (panel length $\geq 12$) and flipping positive in short-survivors (Table~\ref{tab:fake-stars-robustness}), leaving the user-disengagement and selective-survival readings jointly compatible.
...
Robustness check R2 addresses this alternative in two complementary slices (Table~\ref{tab:fake-stars-robustness}).
The calendar-period slice shows that the headline positive short-run coefficient is significant only during the 2024 LLM-surge window and statistically indistinguishable from zero before and after it.
The domain slice shows that the pooled coefficient on labeled repositories ($+0.067^{***}$) hides an order-of-magnitude variation across topics: the short-run effect is significant in \texttt{tool/application}, \texttt{tutorial/demo}, \texttt{ai}, and \texttt{suspicious}, and null in \texttt{blockchain} and \texttt{web}.
The pattern does not cleanly map to a simple AI/LLM hype story---\texttt{tutorial/demo}, which is not a hype-driven category, has the largest short-run coefficient---so the underlying confounder is more accurately framed as topic-by-time and topic-by-channel hype rather than the 2024 LLM surge alone.
Read together, the two slices establish that the pooled estimate is not a single causal parameter; it is an average over heterogeneous within-period and within-domain effects, and topic confounding remains a concern with multiple possible channels.
...
Robustness check R3 swaps the dependent variable and refits a panel-FE autoregression with $\log \textit{fake}_{i,t}$ as the outcome and lagged real-star intensity as the regressor.
...
Intuitively, when prior real-star momentum drives both the maintainer's choice to buy and the next month's organic real-star arrivals, the regression credits those common-cause arrivals to the fake-star coefficient---the estimator reports a $\hat\beta_1$ that is larger than the true causal effect of buying alone.
...
Neither concern is operationalized as a robustness check.
...
The estimate is therefore best understood as the effect of \emph{detectable} fake-star buying on detectable real-star arrivals; the policy recommendation against buying fake stars applies for non-sophisticated buyers (the dominant case in the chapter's data) but may not extend to actors with the resources to evade detection.

\begin{table}[t]
    \footnotesize
    \centering
    \caption{Headline fake-star coefficients across the baseline AR(2) FE specification and the three robustness checks executed for Alternatives 1--3.
    Cells report $\hat\beta$ with HC1 robust standard errors in parentheses; significance codes $^{*}p<0.05$, $^{**}p<0.01$, $^{***}p<0.001$, $^{.}p<0.1$.
    The reverse-causality check R3 swaps the dependent variable to $\log \textit{fake}_{i,t}$ and is reported separately in the prose of Alternative~3.}
    ...
    \midrule
    Baseline AR(2) FE & 12{,}738 & 1{,}042 & $+0.074^{***}$ (0.013) & $+0.029^{*}$ (0.011) & $-0.045^{**}$ (0.014) \\
    \midrule
    \multicolumn{6}{l}{\emph{Alternative 1, R1: survivor-length stratification}} \\
    Panel length $\geq 12$ & 10{,}751 & 394 & $+0.078^{***}$ (0.014) & $+0.009$ (0.015) & $-0.045^{***}$ (0.013) \\
    Panel length $< 12$ & 1{,}987 & 648 & $+0.078^{**}$ (0.024) & $+0.063^{***}$ (0.017) & $+0.061^{*}$ (0.030) \\
    \midrule
    \multicolumn{6}{l}{\emph{Alternative 2, R2: topic stratification (calendar-period slice)}} \\
    Month $<$ 2024-01 & 7{,}951 & 419 & $+0.031$ (0.055) & $-0.068^{.}$ (0.038) & $-0.054^{**}$ (0.020) \\
    2024-01 $\leq$ month $<$ 2024-09 & 3{,}580 & 841 & $+0.080^{***}$ (0.014) & $+0.063^{***}$ (0.013) & $+0.021$ (0.021) \\
    Month $\geq$ 2024-09 & 1{,}207 & 479 & $+0.030$ (0.033) & $+0.011$ (0.026) & $-0.072^{.}$ (0.042) \\
    \midrule
    \multicolumn{6}{l}{\emph{Alternative 2, R2: topic stratification (domain slice)}} \\
    Labeled repositories (pooled) & 5{,}198 & 375 & $+0.067^{***}$ (0.019) & $+0.021$ (0.017) & $-0.076^{***}$ (0.020) \\
    \quad domain $=$ blockchain & 1{,}241 & 51 & $+0.023$ (0.046) & $-0.019$ (0.032) & $-0.069^{***}$ (0.018) \\
    \quad domain $=$ web & 895 & 48 & $+0.003$ (0.048) & $+0.004$ (0.024) & $-0.054^{.}$ (0.031) \\
    \quad domain $=$ tool/application & 683 & 54 & $+0.120^{*}$ (0.047) & $-0.040$ (0.051) & $-0.171^{**}$ (0.057) \\
    \quad domain $=$ ai & 626 & 74 & $+0.099^{*}$ (0.047) & $+0.018$ (0.038) & $-0.042$ (0.051) \\
    \quad domain $=$ suspicious & 542 & 55 & $+0.068^{*}$ (0.031) & $+0.155^{***}$ (0.044) & $-0.034$ (0.082) \\
    \quad domain $=$ tutorial/demo & 426 & 39 & $+0.192^{**}$ (0.064) & $+0.085$ (0.054) & $+0.024$ (0.055) \\
    \bottomrule
    \end{tabular}
\end{table}

\subsection{Causal Credibility Summary}
\label{sec:fake-stars-credibility-summary}

The four alternatives jointly support a relatively weak causal interpretation of the chapter's RQ4 findings.
...
Still, \emph{the discussion's policy recommendation against buying fake stars survives this scrutiny.}
Ultimately, the credibility of the RQ4's causal interpretation rests primarily on A1 (strict exogeneity, with reverse causality biasing $\hat\beta_1$ upward through R3), A2 (no time-varying topic-by-time confounders, partially bounded through R2), and A5 (panel completeness, partially bounded through R1)---the assumptions the design's construction does not guarantee.

\begin{summary-rq}
\textbf{Causal credibility, in plain language:} The fake-stars study has a relatively weak causal design among the three empirical chapters in this thesis because the maintainer's purchase decision is endogenous and the three assumptions all require empirical argument; nonetheless, the chapter's policy recommendation against buying fake stars survives because the residual biases on the short-run estimate point upward---the headline recommendation in this paper ``buying fake stars is ineffective'' is still strong and credible despite the limitations.
\end{summary-rq}
