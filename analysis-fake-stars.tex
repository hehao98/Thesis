...
A mechanical selection artefact is equally consistent: the repositories with the largest fake-star histories tended to be deleted earliest, so the cases pulling the long-run coefficient negative are precisely those whose campaigns failed to convert into sustained attention before exit.
...
Another separate sub-mechanism of survivorship concerns whether GitHub retroactively removes stars from heavily faked repositories, generating part of the long-run negative coefficient through platform action rather than user behaviour.
...
We pair the first three with a concrete robustness check that operationalises the response, and explain why the fourth is not amenable to a within-sample test.
We summarise the headline coefficients in Table~\ref{tab:fake-stars-robustness}. %; full scripts and results are available in the Thesis repository \Code{https://github.com/hehao98/Thesis}.
...
The domain slice shows that the pooled coefficient on labelled repositories ($+0.067^{***}$) hides an order-of-magnitude variation across topics: the short-run effect is significant in \texttt{tool/application}, \texttt{tutorial/demo}, \texttt{ai}, and \texttt{suspicious}, and null in \texttt{blockchain} and \texttt{web}.
...
\begin{table}[t]
    ...
    Labelled repositories (pooled) & 5{,}198 & 375 & $+0.067^{***}$ (0.019) & $+0.021$ (0.017) & $-0.076^{***}$ (0.020) \\
