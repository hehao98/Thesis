...
In human-computer interaction (HCI), controlled experiments comparing interface designs, interaction techniques, or visualization methods are standard practice; the field inherited experimental traditions from psychology and human factors engineering~\cite{DBLP:conf/chi/Vaithilingam0G22}.
...
In industry, A/B testing has become ubiquitous for user experience research~\cite{kohavi2020trustworthy}.
...
These experiments provide clean causal evidence for product decisions because randomization eliminates the selection effects that plague observational comparisons.
...
The study found that AI tools did not significantly improve completion time for experienced developers---a result that diverged from earlier experiments showing large productivity gains.
Such experiments provide clean causal evidence while maintaining relevance to real-world practice.

While the appeal of RCTs in software engineering is clear, RCTs also face significant practical limitations in software engineering contexts.
First, many important questions resist randomization and are impratical for an RCT design.
It would be prohibitively expensive, unethical, or just plainly infeasible, to run an experiment to randomly assign programming languages to projects in production, assign developers to adopt tools they resist, or experimentally manipulate organizational culture.
Second, practical constraints limit experiment duration and scale: Software projects unfold over months or years, but experiments typically run for days or weeks. 
...
Yet, these limitations do not diminish the value of RCTs when they are feasible and well-executed---they provide an important source of triangulation at one side of trade-offs: highest internal validity and limited external validity.
...
Recent examples illustrate this potential: Fang et al.\ used a DiD design to estimate the causal effect of tweets on open-source project popularity and contributor growth~\cite{DBLP:conf/icse/FangLHV22}, and Chapter~\ref{chp:cursor} of this thesis applies DiD with staggered adoption to estimate the effects of Cursor AI on development velocity and software quality.
...
Identifying genuine RDD opportunities in software engineering remains an open challenge for future research.
