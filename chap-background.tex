...
We illustrate each through the programming language and defect proneness example and the DAG in Figure~\ref{fig:pl-dag}.
...
The preceding sections developed the technical foundations: the potential outcomes framework for defining estimands and assumptions, graphical causal models for encoding and scrutinizing those assumptions, and a taxonomy of threats that can undermine any study.
This section draws these threads together into the pragmatic approach advocated by this thesis.
We first show how the four steps of causal credibility assessment articulated in the thesis statement (Chapter~\ref{chp:intro}) map onto the concepts introduced in this chapter, illustrating each step with the programming language and defect proneness example.
We then discuss two broader considerations---the internal--external validity trade-off and the hierarchy of evidence---that guide researchers in situating their contributions within the landscape of causal evidence.

\subsection{Causal Credibility Assessments in Four Steps}

The thesis statement argues that empirical software engineering research needs an explicit framework for \emph{causal credibility assessments}: A structured process for articulating why a study's findings should (or should not) be interpreted causally.
We propose four steps, each grounded in the formal tools introduced earlier.

\paragraph{Step 1: Make the causal theory explicit as a DAG.}
Before fitting any model, the researcher should encode the assumed data-generating process as a directed acyclic graph.
As discussed in Section~\ref{sec:formalism}, a DAG makes every causal assumption visible: Each arrow is a substantive claim; each absent arrow asserts the absence of a direct effect.
This step forces the researcher to confront questions that regression specifications leave implicit: What are the confounders?
Which variables are mediators that should not be conditioned on?
What alternative structures could generate the same observed associations?
...
In the language--defect setting, the estimand might be the ATE of adopting a statically-typed language on defect density.
...
Acknowledging these limitations does not weaken the contribution---it strengthens it by demonstrating that the researcher understands the evidence they are providing and the conditions under which their conclusions hold.
...
The final step connects directly to the ``alternative DAGs'' discussion in Section~\ref{sec:formalism}.
The researcher should consider rival causal structures that could also explain the observed findings and assess whether the data can discriminate among them.
...
Even after a careful credibility assessment, every study faces a fundamental tension between internal validity (did we correctly estimate the causal effect?) and external validity (does the effect generalize?).

In the language--defect setting, a randomized experiment---assigning developers to implement the same task in different languages---would achieve strong internal validity: Random assignment eliminates confounding by construction.
...
Conversely, a large-scale observational study of GitHub repositories captures real-world variation and diverse contexts but cannot guarantee that the estimated language effect is not driven by unobserved selection.

This tension is not a flaw to be resolved but a reality to be navigated.
No single study achieves perfection on both dimensions.
The pragmatic response is to pursue \emph{triangulation}: accumulating evidence across complementary designs whose strengths compensate for each other's weaknesses.
...
When they diverge, the discrepancy itself is informative---perhaps effects vary with context in ways that merit investigation.
...
At the top sit randomized experiments with representative samples, which provide strong internal and external validity simultaneously.
Below these are quasi-experimental designs---regression discontinuity, instrumental variables, well-executed difference-in-differences---that achieve internal validity under assumptions that can be partially tested (e.g., parallel pre-trends, no manipulation at thresholds).
