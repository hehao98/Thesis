...
However, it offers limited guidance on a practical question that researchers inevitably face: \emph{Which} variables should be conditioned on, and which should be left out?
...
Under that alternative structure, no amount of covariate adjustment would recover a causal effect because none exists.
...
When two researchers disagree about whether a study's findings are causally interpretable, the DAG framework channels the debate into questions about specific edges and paths rather than vague objections about ``lurking variables.''
...
The actual estimation is performed by specific statistical methods---difference-in-differences, regression discontinuity, instrumental variables, matching---each of which makes assumptions that can be understood and justified within either framework.
...
Graphical models excel at encoding \emph{why} the assumptions are plausible in the specific research context and at identifying \emph{what} could be missing (omitted confounders, unblocked back-door paths).
