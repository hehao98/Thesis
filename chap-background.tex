...
\begin{itemize}[leftmargin=15pt]
    ...
    \item \textbf{The Graphical Causal Models:} Developed by Pearl~\cite{pearl2009causality}, this framework represents causal relationships as directed acyclic graphs (DAGs), where nodes are variables and edges represent direct causal effects.
...
Recall the question: Does the choice of programming language causally affect defect-proneness?
...
While intuitive, this approach has fundamental limitations that often render it insufficient for causal claims unless the researcher approaches it with extreme caution (Section~\ref{sec:when-can-regression}).
...
A regression with 100 covariates is not better than a regression with five in terms of causal credibility.
...
Nevertheless, a researcher can still argue for the plausibility of conditional ignorability by encoding the assumed data-generating process as a causal DAG and showing that the covariates in $X$ satisfy the back-door criterion~\cite{pearl2009causality}.
The researcher can further probe its robustness through sensitivity analysis techniques such as Rosenbaum bounds~\cite{rosenbaum2002observational}, $E$-values~\cite{vanderweele2017sensitivity}, or coefficient stability tests~\cite{oster2019unobservable}.

\paragraph{Summary of Other Methods.}
...
Table~\ref{tab:methods-summary} summarizes the methods most frequently used in applied research, including the two discussed above and several quasi-experimental designs that appear in later chapters.
...
\begin{tabular}{@{} p{4.1cm} p{2.2cm} p{8.5cm} @{}}
\toprule
...
This is the sense in which the four-step process demonstrated in this thesis calls for researchers to ``derive an explicit causal theory from domain knowledge'' and to ``thoughtfully navigate alternative explanations (i.e., alternative causal structures or causal DAGs) that may also be supported with the current results.''
DAGs are one particularly useful way to achieve this explicitness, as discussed in Section~\ref{sec:pragmatic}.
...
The actual estimation is performed using specific designs and statistical methods---difference-in-differences, regression discontinuity, instrumental variables, and matching---each of which makes assumptions that can be understood and justified within either framework.
...
We first show how the four steps of causal credibility assessment introduced in Chapter~\ref{chp:intro} map onto the concepts introduced in this chapter and illustrate each step using the programming language and defect-proneness example.
...
As argued in the thesis statement, causal credibility assessments are feasible in empirical software engineering through explicit articulations of causal theory and assumptions.
The four-step process introduced in Chapter~\ref{chp:intro} operationalizes this idea as a structured process for articulating why a study's findings should---or should not---be interpreted causally.
Each step is grounded in the formal tools introduced earlier in this chapter.
...
In practice, empirical software engineering papers that engage with causal reasoning typically embed their theoretical assumptions in textual discussions---describing potential confounders in prose, acknowledging threats to validity, and arguing informally about causal pathways.
While such textual articulation can convey the researcher's causal reasoning, it makes assumptions difficult to evaluate systematically: Readers must reconstruct the assumed causal structure from prose scattered across sections, and it is easy to overlook inconsistencies, missing confounders, or implicit conditioning on mediators and colliders.
A directed acyclic graph (DAG) provides a more disciplined alternative by encoding the assumed data-generating process as a formal structure (Section~\ref{sec:formalism}): Each arrow is a substantive claim; each absent arrow asserts the absence of a direct effect.
This visual explicitness opens the assumptions to scrutiny in a way that prose alone cannot: The absence of confounders or the inclusion of colliders in a model specification would become apparent once an explicit causal DAG is defined by the researcher.

The importance of an explicit DAG varies with the research design.
For studies relying on regression adjustment or selection-on-observables---where the credibility of causal claims hinges entirely on whether the correct set of variables is conditioned on---a DAG is particularly valuable, because the back-door criterion (Section~\ref{sec:formalism}) provides a formal basis for determining which covariates to include and which to exclude.
Conditioning on a mediator, a collider, or an irrelevant variable can introduce bias that textual reasoning alone may not detect.
For quasi-experimental designs such as difference-in-differences or regression discontinuity, where identification derives from the study design itself rather than from covariate selection, specifying a complete DAG is less critical: The key assumptions (e.g., parallel trends, continuity at the threshold) are properties of the research setting that can be argued and tested without fully enumerating every variable in the data-generating process.
Even in these settings, however, a partial DAG can help clarify which confounders the design does and does not address, and what alternative explanations remain plausible in each specific research setting.

Crucially, the plausibility of any explicit causal theory---whether expressed as a DAG or articulated in prose---rests on \emph{domain knowledge}~\cite{pearl2009causality}, and in software engineering, this is largely captured in the prior literature, especially descriptive and qualitative research.
Surveys, case studies, ethnographies, and grounded-theory studies of development practice are what tell us \emph{which} factors shape language adoption, team behaviour, and defect introduction~\cite{DBLP:conf/icse/Devanbu0B16}; without this foundation, there is no principled basis for deciding which variables belong in the causal theory and which relationships connect them.
A causal theory whose structure cannot be traced back to substantive findings or well-documented domain expertise is little more than speculation, whether it is dressed in formal DAG notation or buried in prose.
Prior descriptive and qualitative work, therefore, plays an indispensable role in causal inference: It supplies the substantive justification for the completeness of the assumed causal structure and, by extension, for the validity of every downstream identification argument.

For the language--defect problem, Figure~\ref{fig:pl-dag} encodes the theory as a DAG: Language affects Defect Proneness through Tooling/Type Safety, while Domain, Team Skill, Org Culture, and Codebase Age confound the relationship.
...
Because this example relies on a selection-on-observables identification strategy, the DAG is essential: It reveals precisely which confounders must be conditioned on (Domain, Team Skill, Org Culture, Codebase Age) and which variables must be excluded (Tooling/Type Safety as mediator, Project Complexity as non-confounder) for the regression coefficient to carry a causal interpretation.

\paragraph{Step 2: Define the causal estimate of interest and specify the identifying assumption.}
With the causal theory articulated, the researcher defines the causal estimate of interest using the potential outcomes framework (Section~\ref{sec:formalism}) and specifies the assumption under which the estimate would carry a causal interpretation.
In the language-defect setting, the causal estimate of interest might be the ATE of adopting a specific programming language with respect to the average defect density over projects in all programming languages.
...
Each method rests on specific assumptions that should be stated explicitly and connected back to the causal theory, and the researcher should consult the methodological literature to understand when those assumptions are tenable and what diagnostics are available to assess them.
...
Compounding this challenge, data collection, metric design, and measurement infrastructure in software engineering are themselves fraught with difficulty: Key constructs---defect proneness, productivity, code quality---lack universally accepted definitions~\cite{DBLP:journals/software/KitchenhamP96,DBLP:journals/queue/ForsgrenSMZHB21}; the proxies available from mining software repositories (bug-fix commits, issue counts, static-analysis warnings) each capture only a narrow, noisy slice of the underlying concept~\cite{DBLP:journals/ese/KalliamvakouGBS16}; and the tools and heuristics used to extract these metrics (e.g., commit-message classifiers, static analysis engines whose precision varies across languages) may themselves introduce systematic artifacts that interact with the treatment under study.
...
At the bottom are descriptive and correlational studies, which document patterns and generate hypotheses but cannot answer causal effects definitively.
...
Imperfect evidence, accumulated thoughtfully across multiple studies and methods, guides decisions far better than intuition and conventional wisdom alone---and explicit credibility assessments, such as the four-step process demonstrated in this thesis, help ensure that the strength and limitations of each piece of evidence are communicated honestly.
