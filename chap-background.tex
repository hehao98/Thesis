...
Before fitting any model, the researcher should encode the assumed data-generating process as a directed acyclic graph (~\ref{sec:formalism}) so that every causal assumption is visible: Each arrow is a substantive claim; each absent arrow asserts the absence of a direct effect.
...
Committing to this DAG---and presenting it to reviewers---opens the assumptions to scrutiny in a way that a regression table alone cannot.

\paragraph{Step 2: Define the causal estimand and specify the identifying assumptions.}
...
This step also determines the statistical method.
If the researcher has access to panel data where developers or repositories use multiple languages over time, developer and repository fixed effects can absorb time-invariant confounders.
If the setting features staggered language migration events, a difference-in-differences design can exploit within-project variation.
Each method rests on specific assumptions---parallel trends, no anticipation, exogenous timing---that should be stated explicitly and connected back to the DAG.

\paragraph{Step 3: Honestly acknowledge and mitigate limitations.}
No observational study achieves perfect identification.
The threats catalogued in Section~\ref{sec:threats}---confounding from unmeasured variables, selection bias, measurement error, SUTVA violations---apply to every study, and the researcher's task is to assess their severity honestly rather than assume them away.
In the language--defect example, key limitations may include: Conditional ignorability may be violated if developer motivation or risk tolerance (absent from the DAG) drives both language choice and code quality; the treatment ``programming language'' is measured imprecisely in polyglot repositories; and the outcome ``defect proneness'' proxied by bug-fix commits is noisy and potentially correlated with language-specific commit conventions.
Where possible, limitations should be mitigated: Sensitivity analysis can bound the impact of omitted confounders (Section~\ref{sec:threats}), and triangulating across multiple outcome measures can guard against construct-dependent conclusions.
...
For instance, a critic of the language--defect study might propose an alternative DAG in which Language has no causal effect and the entire association is driven by Team Skill and Org Culture.
Under this alternative, even perfect adjustment for the confounders in Figure~\ref{fig:pl-dag} would yield a null effect, so a non-null estimate after adjustment would count against the alternative.
Another critic might add an unmeasured confounder---developer motivation---that opens a new back-door path.
In this case, the researcher cannot rule out the alternative from data alone, but can report how strong the confounding from motivation would need to be to explain away the estimated effect (e.g., via $E$-values~\cite{vanderweele2017sensitivity} or Rosenbaum bounds~\cite{rosenbaum2002observational}).
The goal is not to eliminate all alternative explanations---an impossible standard---but to engage with the most plausible ones transparently and show that the findings are robust to reasonable alternatives.
...
At the bottom are descriptive and correlational studies, which document patterns and generate hypotheses but cannot answer ``what if'' questions about interventions.
