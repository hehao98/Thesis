...
Two complementary frameworks dominate modern causal inference.
The \emph{potential outcomes framework}, developed by Neyman and formalized by Rubin, defines causation through counterfactual comparison: What would have happened under alternative treatments?
The \emph{graphical causal model framework}, developed by Pearl, represents causal relationships as directed graphs and provides algorithmic tools for determining when causal effects can be estimated from observational data.
Though they differ in formalism and emphasis, the two frameworks are mathematically compatible and often yield identical conclusions.
...
The potential outcomes framework begins with a deceptively simple question: What does it mean to say that $X$ causes $Y$?
...
The difficulty---which Holland famously termed the ``fundamental problem of causal inference''---is that $\tau_i$ is inherently unobservable.
...
The entire enterprise of causal inference consists of finding ways to fill in these missing potential outcomes---or, more precisely, to estimate population summaries without needing to observe them individually.
...
High-ability developers may adopt new tools precisely because they are high-ability; comparing their outcomes to non-adopters confounds the tool's effect with pre-existing ability differences.
...
\begin{enumerate}[leftmargin=15pt]
    ...
    It is inherently untestable---we cannot verify that all confounders have been measured because we cannot observe the unmeasured ones.
    Its plausibility rests entirely on substantive knowledge of the data-generating process.
    ...
    Overlap is testable: We can examine whether the distributions of covariates (or propensity scores) in treated and control groups have common support.
...
A path between $X$ and $Y$ is \emph{blocked} by a set $Z$ if either:
\begin{itemize}[leftmargin=15pt]
    \item The path contains a non-collider (a variable with at most one arrow pointing in) that is in $Z$, or
    \item The path contains a collider that is \emph{not} in $Z$ and has no descendants in $Z$.
\end{itemize}
...
\paragraph{Connecting the Frameworks.}
Though developed independently, the potential outcomes and graphical frameworks are mathematically compatible.
The potential outcomes condition of ignorability---$(Y^1, Y^0) \perp D \mid X$---corresponds to d-separation in an appropriately constructed graph: $X$ d-separates $D$ from the potential outcomes when $X$ blocks all back-door paths.
The two frameworks offer complementary strengths: Potential outcomes provide a rigorous language for estimands and clarify what randomization achieves; graphs provide visual representation of assumptions and algorithmic tools for determining adjustment sets.
Fluency in both is valuable for the applied researcher.
