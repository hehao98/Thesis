...
\emph{(1) Causal theory:} The causal theory is made explicit through the counterfactual simulation structure rather than a traditional DAG: the treatment (pinning all direct dependencies) affects security and maintenance outcomes through npm's dependency resolution mechanism, with project- and time-specific heterogeneity as confounders controlled through fixed-effects panel regression.
The simulation framework encodes the assumed data-generating process---how version constraints interact with the npm resolver, transitive dependencies, and the evolving ecosystem---making the causal assumptions transparent and scrutinizable.
...
\emph{(1) Causal theory:} The assumed causal mechanism is that acquiring fake stars increases a repository's displayed star count, which in turn attracts genuine attention and real stars through the ``rich get richer'' dynamic prevalent in social platforms.
This theory is contrasted with an alternative causal structure---where both fake star acquisition and low real star attraction are jointly driven by unobserved maintainer characteristics (e.g., inexperience or project quality)---which is explicitly considered in the analysis.
...
\emph{(1) Causal theory:} The causal theory posits that Cursor adoption (treatment) affects development velocity and software quality (outcomes) through the mechanism of AI-augmented code generation.
Potential confounders---team culture, project maturity, developer expertise, and application domain---are controlled by the DiD design's repository-fixed effects (absorbing all time-invariant confounders) and month-fixed effects (absorbing all repository-invariant temporal factors), supplemented by propensity score matching on pre-adoption trajectories.
...
Each makes its causal theory explicit---whether through the counterfactual simulation structure encoding how version constraints interact with npm's dependency resolution (pinning), the panel autoregressive model capturing the hypothesized promotional dynamics and contrasting it with alternative causal structures (fake stars), or the DiD design with matched controls encoding the assumed treatment mechanism and its confounders (Cursor).
