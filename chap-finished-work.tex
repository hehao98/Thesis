...
Each study challenges a widely held software engineering belief---what I have termed a \emph{software engineering myth}---by employing rigorous causal reasoning, making identifying assumptions explicit, and acknowledging methodological limitations honestly.
...
\section{The Impact of Dependency Pinning on Supply Chain Security}
...
Security practitioners increasingly advocate for dependency pinning as protection against malicious package updates---the rationale being that pinned dependencies prevent automatic installation of compromised versions.
...
The study curates a balanced dataset of 10,000 npm packages and 10,000 GitHub repositories using npm, ensuring representation of both libraries and applications.
Projects are selected based on popularity (top 5\% downloads for packages, top 1\% stars for repositories), active maintenance (commits within two years), and having production dependencies to analyze.
...
\begin{enumerate}[leftmargin=15pt]
    \item \textsf{Number of floating dependencies}: The attack surface for malicious updates---packages where a compromised version would be automatically installed.
    \item \textsf{Number of automatic updates}: Exposure to breaking changes, assuming such changes occur uniformly among releases.
    \item \textsf{Number of security vulnerabilities}: Known vulnerabilities in the dependency graph based on GitHub Advisory entries.
    \item \textsf{Number of outdated dependencies}: Direct dependencies with newer versions available, indicating maintenance burden.
    \item \textsf{Number of bloated dependencies}: Packages with multiple versions simultaneously present, indicating dependency conflicts.
\end{enumerate}

\paragraph{Statistical Analysis}
The study employs \emph{fixed-effects panel regression}~\cite{bruderl2015fixed} to estimate the causal impact of pinning while controlling for unobserved project-specific and time-specific heterogeneity.
...
\end{equation}
This specification allows the data to reveal whether pinning's impact differs for projects with small versus large dependency graphs---a critical nuance that prior qualitative studies could not address.
...
While pinning does reduce automatic updates (as expected), the study reveals a counter-intuitive finding: \emph{pinning direct dependencies can actually increase the attack surface for malicious package updates} in projects with large dependency graphs.
This surprising result stems from how npm's dependency resolver handles version conflicts---pinning creates more opportunities for duplicate package versions, each representing an additional attack vector.
...
\section{The Impact of Fake GitHub Stars on Repository Promotion}
...
However, this reliance creates perverse incentives: if star counts influence decision-making, actors have motivation to manipulate them.
...
These limitations are honestly articulated, allowing readers to calibrate their confidence appropriately.
...
\section{The Impact of Cursor AI on Open-Source Projects}
...
Each limitation is honestly acknowledged, embodying the pragmatic stance that transparent uncertainty communication is preferable to false claims of certainty.
...
Panel GMM analysis reveals that this accumulated technical debt subsequently reduces future development velocity, creating a self-reinforcing cycle where initial productivity surges give way to maintenance burdens.

Notably, the code complexity increase persists even after controlling for velocity dynamics, providing evidence that code generated with LLM agents may be inherently more complex than human-written code---a ``complexity debt'' that compounds over time.
...
Cross-sectional comparisons or short-term experiments would have produced misleading conclusions about AI coding assistant effectiveness.
...
The pragmatic approach recognizes that no observational study achieves the certainty of a randomized controlled trial, but maintains that carefully designed quasi-experiments with transparent assumptions can nonetheless build trustworthy evidence incrementally.
